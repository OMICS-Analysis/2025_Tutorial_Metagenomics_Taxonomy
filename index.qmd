...
Para utilizar [`HUMAnN 3.0`](https://github.com/biobakery/humann/?tab=readme-ov-file#3-test-the-install) vamos a requerir instalar primero a [`MetaPhlAn`](https://huttenhower.sph.harvard.edu/metaphlan3/), estas herramientas también nos permitirán hacer un peril taxonómico de muestras de metagenomica partiendo de los archivos FASTQ.

```{bash, eval=FALSE}
pip install humann
```

Vamos a descargar la base de datos que utilizaremos para le perfil taxonómico (`Base de datos filtrada UniRef90 EC`), vamos a utilizar el mismo directorio `2025_metagenomics_direct/dbs`:

```{bash, eval=FALSE}
humann_databases --download uniref uniref90_ec_filtered_diamond ./
```

También necesitamos descargar la base de datos `ChocoPhlAn` (Este paso toma su tiempo, paciencia):
...
--output 3_humann2 \
--metaphlan bin/MetaPhlAn/metaphlan.py
```

Ojo, para evitar problemas de compatibilidad usar la siguiente vía de instalación de MetaPhlan, además qué te ayudará a futuro a ejecutar esta herramienta individualmente:
...
Descargar las bases de datos correspondientes a MetaPhlan (Necesitarás `~30GB` para este paso, tómate tu tiempo, va a tardar):

```{bash, eval=FALSE}
metaphlan --install
```

### Extra: Alternativa con ambientes conda

Para tener compatibilidad de todas las herramientas que utiliza HUMAnN te recomiendo crean un ambiente con `conda` o `mamba`.

```{bash, eval=FALSE}
mamba create -n humann_env -c conda-forge -c bioconda \
  humann bowtie2 diamond vsearch metaphlan -y
```
...
humann_config --update database_folders nucleotide dbs/chocophlan
humann_config --update database_folders protein dbs/uniref
```

*Extra: para poder lograr una correcta cohersión de versiones y parametros de `metaphlan` se tuvieron que hacer los siguientes ajustes, si usas ambiente conda*:

```{bash, eval=FALSE}
#| code-fold: true
#| code-summary: "Correciones a metaphlan"

cd ~/miniconda3/envs/humann_env/bin
mv metaphlan metaphlan.real

cat > metaphlan <<'EOF'
#!/usr/bin/env bash
# Wrapper para compatibilizar llamadas antiguas (--bowtie2out) con MetaPhlAn4 (--mapout)

REAL_METAPHLAN="$(dirname "$0")/metaphlan.real"
...
exec "$REAL_METAPHLAN" "${newargs[@]}"
EOF

chmod +x metaphlan
which metaphlan
metaphlan --help | sed -n '1,40p'

```

Se recomienda descargar la base de datos de metaphlan:
